...
model = torch.hub.load("ultralytics/yolov5", "custom", path="path/to/CBAMbest.pt")  # local model
model = torch.hub.load("path/to/yolov5", "custom", path="path/to/CBAMbest.pt", source="local")  # local repo
```
