...
from ultralytics.nn.modules import SPP, RepConv, Bottleneck, C2f
...
class BiLevelRoutingAttention(nn.Module):
    ...
    """
    def __init__(self, dim=8, n_win=7, num_heads=8, qk_dim=None, qk_scale=None,
...
class BRAttention_Bottleneck(Bottleneck):
    def __init__(self, c1, c2, shortcut=True, g=1, k=(3, 3), e=0.5):  # ch_in, ch_out, shortcut, groups, kernels, expand
        super().__init__(c1, c2, shortcut, g, k, e)
        self.bra = BiLevelRoutingAttention(c2)

    def forward(self, x):
        return x + self.bra(self.cv2(self.cv1(x))) if self.add else self.bra(self.cv2(self.cv1(x)))


class C2f_BRAttention(C2f):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        super().__init__(c1, c2, n, shortcut, g, e)
        self.c = int(c2 * e)  # hidden channels
        self.m = nn.ModuleList(BRAttention_Bottleneck(self.c, self.c, shortcut, g, k=((3, 3), (3, 3)), e=1.0) for _ in range(n))